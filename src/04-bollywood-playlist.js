...
export function buildPlaylist(songs, maxDuration) {
  if (!Array.isArray(songs) || typeof maxDuration !== 'number' || maxDuration <= 0) return {
    count: 0,
    totalDuration: 0
  }
  ...
  while (i < songs.length) {
    ...
    else if (typeof song === 'number' && song !== NaN && song > 0)  { count++; i++; totalDuration += song }
    else i++;
  }
...
console.log(buildPlaylist([100, -50, 200, 150], 350))
