...
export function calculateEMI(principal, monthlyRate, emi) {
  if (typeof principal !== 'number' || typeof monthlyRate !== 'number' || typeof emi !== 'number' || principal <= 0 || monthlyRate <= 0 || emi <= 0 || emi <= principal * monthlyRate) return {
    months: -1,
    totalPaid: -1,
    totalInterest: -1
  };
  ...
  while (principal >= 0) {
    int = (principal < emi ? principal : emi) * monthlyRate;
    principal += int;
    principal -= emi;
    months++;
    totalPaid += principal < emi ? principal : emi;
    totalInterest += int
  }
